			...
			<div class="box-body">
	            ...
	            <div class="form-group has-feedback">
	             ...
	             <div class="form-group">
					...
					<div class="col-xs-9">
	                    <tr>${card.name }</tr>&nbsp&nbsp<button type="button" class="btn btn-warning btn-xs" onclick="addPicture();"><i class="fa fa-paste"></i>拍照</button>
						    <img id="webcamImg1" src="" /><p>
...
function save(){
	...
	if(validate.form()){
		...
		$.ajax({
			type:'post',
			url:'<%=ctx %>/user/save',
			data:{'id':id,'account':account,'password':password,'name':name,'tel':tel,'email':email,'customerid':customerid,'facedate':facedate},
			dataType:'text',
			async:false,
