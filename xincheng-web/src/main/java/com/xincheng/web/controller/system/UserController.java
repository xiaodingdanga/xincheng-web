...
public class UserController extends BaseController{
	 ...
	 * @param modal
	 */
	@RequestMapping(value="/save",produces={"application/text;charset=UTF-8"})
	public @ResponseBody String save(HttpServletRequest request,Model modal){
		//获取参数
		Map<String,Object> paramap=this.getParameter();
		String id=paramap.get("id").toString();
		if(id.equals("")){//保存
			String string = userService.save(paramap);
			return string;
		}else{//更新
			String string = userService.update(paramap);
			return string;
		}
	...
	public ModelAndView toedit(HttpServletRequest request,Model modal){
		...
		Map<String, Object> faceMap = userFaceService.findByUserId(userid);
		if(faceMap!=null){
			String facedate=faceMap.get("facedate")==null?"":faceMap.get("facedate").toString();
			usermap.put("facedate",facedate );
		}
