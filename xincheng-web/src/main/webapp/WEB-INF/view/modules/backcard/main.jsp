...
function init(height){
    ...
    jQuery("#table").jqGrid({
		...
		url:'<%=ctx%>/backcard/getDataList',//这是Action的请求地址 
        ...
        gridComplete:function(){
            ...
            for(var i=0;i<ids.length;i++){
                ...
                var editBtn="";//此处会将点击行id传给_edit(id) js函数
                	editBtn+="<button type=\"button\" class=\"btn btn-primary btn-xs\" onclick=\"backcard(\'"+id+"\',\'"+money+"\');\"><i class=\"fa fa-paste\"></i>退卡</button>";
	                editBtn+="&nbsp;&nbsp;";
...
//退卡
function backcard(id,money){
	var dialog=new Dialog();
	dialog.Width = 500;
	dialog.Height = 150;
	dialog.Title="退卡";
	dialog.URL="<%=ctx%>/backcard/toadd?id="+id+"&money="+money;
