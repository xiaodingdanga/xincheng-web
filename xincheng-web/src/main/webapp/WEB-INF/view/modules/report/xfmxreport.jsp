	...
	<div class="col-md-12">
		<div class="box box-default">
			<div class="box-body">
				...
				<div class="row">
					<label class="col-xs-1 control-label" style="padding: 10px 0px 0px 50px;">姓名:</label>
					<div class="col-xs-3" style="padding: 5px 0px 0px 0px;">
...
function init(height){
    ...
    jQuery("#table").jqGrid({
		...
		url:'<%=ctx%>/report/xfmxgetDataList',//这是Action的请求地址 
		postData:{'serialnum':serialnum,'startdate':startdate,'enddate':enddate,'name':name},  
	    mtype:'POST',
	    height:height,
   	    autowidth:true,
        shrinkToFit:true,
        colNames:['id','姓名','会员卡号','消费金额','实际消费金额','余额','油表价','售价','消费日期'],//列名
        colModel:[//列对应的字段
        	...
        	{name:'realprice',index:'realprice',width:100,sortable:false},  
        	{name:'newmoney',index:'newmoney',width:100,sortable:false},  
        	{name:'outprice',index:'outprice',width:100,sortable:false},  
        	{name:'inprice',index:'inprice',width:100,sortable:false},  
...
function query(){
	...
	var enddate =$("#enddate").val();
	var name =$("#name").val();
	var postdata={'serialnum':serialnum,'startdate':startdate,'enddate':enddate,'name':name};
