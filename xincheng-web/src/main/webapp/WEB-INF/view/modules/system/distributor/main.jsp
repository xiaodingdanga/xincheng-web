			...
			<div class="box-body">
				<div class="row">
					<label class="col-xs-1 control-label"
						style="padding: 10px 0px 0px 20px;">姓名:</label>
...
function init(height){
    ...
    jQuery("#table").jqGrid ({
		...
		postData:{'id':'','name':'','tel':'','sex':''},//此数组内容直接赋值到url上，参数类型：{name1:value1…}  ,'isdel':''
	    mtype:'POST',
	    height:height,
   	    autowidth:true,
        shrinkToFit:true,
        colNames:['id','分销员姓名','电话','金额','地址','备注','性别','操作'],//列名   ,'金额'
        colModel:[//列对应的字段
...
function distributorwithdraw(id,money){
	...
	dialog.Height = 150;
	dialog.Title="佣金提现";
	dialog.URL="<%=ctx%>/distributor/distributorwithdrawtoedit?id="+id+"&money="+money;
	dialog.OKEvent=function(){
		var ret=dialog.innerFrame.contentWindow.save();
			if(ret=="ok"){
				dialog.close();
